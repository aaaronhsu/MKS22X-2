  ...
  public static void main(String[]args) {
    if(args.length < 1){
	  ...
	  if (args.length == 3) {//anything exists in test.
		  int[] randDataQuick = Arrays.copyOf(randData, randData.length);
		  Arrays.sort(randData);
		  Quick.quicksort(randDataQuick);

		  if(Arrays.equals(randData,randDataQuick)) {
          	System.out.println("Quick Correct!");
          } else {
         	System.out.println("Quick Not Correct!!!!!!!!!11oneeleven");
          }
	  } else {
		 Quick.quicksort(randData);
	  }
    }
  }
}
